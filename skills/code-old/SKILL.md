---
name: code
description: Orchestrate project code tasks with mandatory confirmation, file-based state, mini-spec-kit, a long-lived Session1 plus short-lived Session2 worker loop, executor dispatch (CC/Codex), provider compatibility, feasibility validation, and isolated test harness patterns.
---
...
- **Explicit change plan**: state exactly which files you plan to touch and what each change is. User must be able to see the full scope before saying GO. "别给我改坏了" means they want to understand the delta before it happens.
- **Lighter confirmation when user names executor**: If the user already chose a specific tool (e.g. "用 agy", "让 Codex 做"), skip the detailed file-by-file change plan. Confirm scope briefly (what to do + which executor) and dispatch directly. The user's explicit tool choice implies they know the scope and just want it executed—over-explaining the plan frustrates them.
...
- Keep that sentence executor-neutral. Do not hard-code `Codex CLI` there, because executor selection may legitimately default to Claude Code and must not create a wording conflict.
- If you need to refer to a specific executor in surrounding text, do so outside the required closing sentence.
- See `references/confirmation-sentence.md` for the executor-neutral closing sentence note.- The confirmation must end with this exact sentence, and it must be placed as its own separate paragraph with one blank line above:
  - `本任务中，我仅负责需求理解确认、已确认 Executor 的一次性任务下达及最后的结果转发，不直接参与任何代码实现。所有代码实现、测试、修改、计划、思考循环迭代、构建、部署及 git 操作必须由已确认 Executor 完成。在已确认 Executor 任务下达之后，不得再查看项目代码、参与实现细节分析或进行任何形式的迭代决策；中间不得因迭代、调用次数、状态变化或任何理由再次介入，不得再次发起工具调用去参与实现、检查、分析、决策或追踪，仅允许转发结果。`
- Keep that sentence executor-neutral. Do not hard-code `Codex CLI` there, because executor selection may legitimately default to Claude Code and must not create a wording conflict.
- If you need to refer to a specific executor in surrounding text, do so outside the required closing sentence.
- See `references/confirmation-sentence.md` for the executor-neutral closing sentence note.
...
- If the user explicitly says a different CLI (agy, aider, etc.), confirm scope and dispatch using that tool instead. Treat the user's explicit tool choice as binding even if it's not one of the standard adapters — the selection section only sets defaults, not a closed set.
...
**Provider compatibility check (required before CC dispatch):** See `references/claude-code-provider-compat.md` for the pre-dispatch checklist — probe model name, verify ANTHROPIC_MODEL, known third-party proxy issues.

**Dual-session marker pattern (for spec/implement separation):** See `references/claude-code-dual-session.md` for the file-marker coordination pattern, CC1/CC2 wrapper scripts, prompt physical isolation, and failure recovery to single-session serial mode.
...
**Feasibility validation pattern:** See `references/codex-feasibility-validation.md` for using Codex CLI in a scratch repo to produce feasibility reports before touching production code.

### Isolated test harness for debugging

See `references/isolated-test-harness.md` for the pattern: when a production data pipeline breaks, build a minimal standalone test program that reproduces the exact pipeline end-to-end before modifying production code. One file, no framework, mirror production structs and logic exactly, save raw response.

### API contract verification across services

See `references/api-contract-verification.md` for the triple-source verification pattern: when a backend acts as a proxy to an upstream API, verify docs vs source code vs live response, and watch for Go JSON decode silently dropping upstream fields.
...
- **Boundary pitfall:** Changing env vars in `docker-compose.yml` (proxy, port, restart policy) is deploy-only. Changing **application source code** (`.py`, `.js`, `.ts`, provider config, any file under `app/`, `src/`, `routes/`, `services/`) is a code change requiring the full workflow — even if it's "just" updating a URL in a provider file. In projects with mini-spec-kit, the deploy-only exception does NOT cover application source files.
- **Cherry-pick pitfall: frontend export/import convention mismatch.** When cherry-picking frontend components between branches that use different component conventions (e.g., STG uses `export default` + `lazy(() => import(...))`, main uses `export function` + `import { Component }`), the cherry-pick itself succeeds but the frontend build fails downstream during `docker compose up --build`. The Go backend compiles fine, masking the issue until the Docker build hits the rollup bundler with `"X is not exported by Y"`. **Fix before push:** after cherry-pick, check the target branch's import style and adjust the component's export style to match. Named exports are more portable across branches; prefer them for cherry-pick targets.
- **Compose port merge pitfall.** When passing multiple `-f` files to `docker compose`, `ports` arrays are merged by appending, not replaced. The override file's ports are added on top of the base file's ports, so both sets of port bindings are created. This causes cross-environment port conflicts when the same base docker-compose.yml is shared by dev/STG/prod. **Fix:** remove `ports` from the base `docker-compose.yml` entirely and put them in environment-specific override files (e.g., `docker-compose.override.stg.yml`, `docker-compose.override.prod.yml`). Verify with `docker compose config | grep published:` before bringing containers up. See `references/compose-port-merge.md`.

- **Branch-environment mismatch pitfall.** When a project has per-environment compose stacks (STG/main) and the compose files build from local source (no pre-pulled images), the **current git branch determines what code the build uses**. Running `docker compose -f docker-compose.yml -f docker-compose.override.stg.yml up --build` while on the `main` branch silently deploys main's code to the STG environment — breaking STG-only features (no-login bypass, mock data, etc.). This is extremely dangerous because the user notices immediately (wrong login page, missing bypass). **Fix before deploy:** always run `git branch --show-current` before any `docker compose up --build`. Verify the current branch matches the target environment. If you're on main and need to rebuild STG, `git checkout STG` first. If you have uncommitted changes, `git stash` before switching.

- **Docker build cache pitfall after branch switch.** Even after switching branches, `docker compose up --build` may use cached `COPY` and `RUN` layers from a previous branch's build. Docker's layer cache key is based on file timestamps and build context — if the context doesn't fully invalidate across branches, Docker uses cached layers and deploys stale code. **Fix:** after switching branches for a different environment, always use `docker compose build --no-cache <service>` or pass `--no-cache` to the build step. When the user reports an environment behaving like a different branch (e.g. "STG shows login button"), suspect a build cache hit first.

- **Branch-environment mismatch pitfall.** When a project has per-environment compose stacks (STG/main) and the compose files build from local source (no pre-pulled images), the **current git branch determines what code the build uses**. Running `docker compose -f docker-compose.yml -f docker-compose.override.stg.yml up --build` while on the `main` branch silently deploys main's code to the STG environment — breaking STG-only features (no-login bypass, mock data, etc.). This is extremely dangerous because the user notices immediately (wrong login page, missing bypass) and is justifiably furious. **Fix before deploy:** always run `git branch --show-current` and `git log -1 --oneline` before any `docker compose up --build`. Verify the current branch matches the target environment before proceeding. If you're on main and need to rebuild STG, `git checkout STG` first. If you have uncommitted changes, `git stash` before switching. Track the original branch so you can restore it afterward if interrupted.
...
For any task that targets a live deployed site, add a real-browser check against the live URL after local build/tests pass. A local green build is not acceptance for a staging outage.
...
Support files:
- `references/live-stg-verification.md` — compact live-site verification sequence and the blank-page/null-user failure pattern.
